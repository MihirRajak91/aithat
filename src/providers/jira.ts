...
export class JiraProvider extends BaseProvider {
  private cache = cacheManager.getCache<any>('jira', {
  ...
  async getRecentlyUpdatedTasks(days: number = 7, limit: number = 20): Promise<RecentTicket[]> {
  ...
  async getUrgentTasks(limit: number = 10): Promise<RecentTicket[]> {
  ...
  // Get tasks due soon
  async getTasksDueSoon(days: number = 7, limit: number = 20): Promise<RecentTicket[]> {
  ...
  async getCurrentSprintTasks(boardId?: number, limit: number = 50): Promise<RecentTicket[]> {
    ...
    return this.getCachedData(cacheKey, async () => {
      ...
      const boards = await this.getBoardsWithMyTasks();
      const allSprintTasks: RecentTicket[] = [];
  ...
  private async executeJQLQuery(jql: string, maxResults: number = 50): Promise<RecentTicket[]> {
    const url = `${this.config.baseUrl}/rest/api/3/search`;
  ...
  private async getMyAssignedTasks(limit: number = 50): Promise<RecentTicket[]> {
  ...
  private isInProgress(status: string): boolean {
    ...
    return inProgressStates.some(state => 
      status.toLowerCase().includes(state.toLowerCase())
    );
  }

  private isReadyToStart(status: string): boolean {
    const readyStates = [
      'to do', 'open', 'new', 'ready', 'backlog', 'selected', 'todo'
    ];
    return readyStates.some(state => 
      status.toLowerCase().includes(state.toLowerCase())
    );
  }
  ...
  private getProjectIcon(boardType?: string): string {
  ...
  async getBoardsWithMyTasks(): Promise<JiraBoard[]> {
    const cacheKey = 'boards-with-tasks';
    return this.getCachedData(cacheKey, async () => {
      try {
        const boards = await this.getBoards(50);
        const boardsWithTasks: JiraBoard[] = [];

        // Process boards in parallel for better performance
        const boardPromises = boards.slice(0, 20).map(async (board: any) => {
          try {
            // Quick check if board has any assigned tasks
            const quickCheck = await this.executeJQLQuery(
              `project = "${board.location?.projectId || board.name}" AND assignee = currentUser() AND status != Done AND status != Closed`, 
              1
            );
            
            if (quickCheck.length > 0) {
              // Get actual count
              const allTasks = await this.executeJQLQuery(
                `project = "${board.location?.projectId || board.name}" AND assignee = currentUser() AND status != Done AND status != Closed`, 
                100
              );
        ...
        boardsWithTasks.push(...results.filter((board: any): board is JiraBoard => board !== null));
  ...
  // Sprint methods
  async getMySprintTasks(boardId: number): Promise<RecentTicket[]> {
  ...
  protected mapToRecentTicket(issue: any): RecentTicket {
    ...
    return {
      ...
      labels: (issue.fields.labels || []).map((label: any) => 
